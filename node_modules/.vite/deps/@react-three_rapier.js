...
import {
  mergeVertices
} from "./chunk-YLLNIKBT.js";
import "./chunk-QH655W3M.js";
import {
  suspend,
  useFrame,
  useThree
} from "./chunk-6U6PNMOF.js";
import {
  BufferAttribute,
  DynamicDrawUsage,
  Euler,
...
var vector3ToRapierVector = (v) => {
  if (Array.isArray(v)) {
    return new GA(v[0], v[1], v[2]);
  } else if (typeof v === "number") {
    return new GA(v, v, v);
  } else {
    const threeVector3 = v;
    return new GA(threeVector3.x, threeVector3.y, threeVector3.z);
  }
};
var quaternionToRapierQuaternion = (v) => {
  if (Array.isArray(v)) {
    return new SA(v[0], v[1], v[2], v[3]);
  } else {
    return new SA(v.x, v.y, v.z, v.w);
  }
};
...
var createColliderFromOptions = (options, world, scale, getRigidBody) => {
  ...
  const desc = xI[options.shape](...scaledArgs);
  ...
  (0, import_react.useEffect)(() => {
    ...
    if (collider) {
      ...
      if (hasCollisionEvent && hasContactForceEvent) {
        collider.setActiveEvents(RA.COLLISION_EVENTS | RA.CONTACT_FORCE_EVENTS);
      } else if (hasCollisionEvent) {
        collider.setActiveEvents(RA.COLLISION_EVENTS);
      } else if (hasContactForceEvent) {
        collider.setActiveEvents(RA.CONTACT_FORCE_EVENTS);
      }
...
var importRapier = async () => {
  let r = await import("./rapier.es-4Q7DMZUP.js");
  await r.init();
  return r;
};
var Physics = (props) => {
  ...
  const eventQueue = useConst(() => new bI(false));
